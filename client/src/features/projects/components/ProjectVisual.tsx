...
import type { ProjectVisual as VisualType } from '../data/projectData'
...
export default function ProjectVisual({ type, isHovered }: ProjectVisualProps) {
      ...
      <div className="absolute inset-0 flex items-center justify-center" aria-hidden="true">
        <svg viewBox="0 0 120 120" className="w-full h-full max-w-[100px] max-h-[100px]">
          <motion.circle
            ...
            strokeWidth="0.5"
            className="text-white/[0.06]"
            animate={isHovered ? { r: 48, stroke: 'rgba(245,247,242,0.12)' } : { r: 45 }}
          ...
          <motion.circle
            ...
            strokeWidth="0.5"
            className="text-white/[0.04]"
            animate={isHovered ? { r: 30, stroke: 'rgba(182,243,74,0.1)' } : { r: 28 }}
            transition={{ duration: 0.5, ease }}
          />
          <motion.circle
            cx="60"
            ...
            fill="currentColor"
            className="text-white/[0.06]"
            animate={isHovered ? { r: 10, fill: 'rgba(182,243,74,0.2)' } : { r: 8 }}
            transition={{ duration: 0.4 }}
          />
          <motion.circle
            cx="60"
            ...
            animate={isHovered ? { fill: 'rgba(182,243,74,0.4)' } : {}}
            transition={{ duration: 0.4 }}
          />
        </svg>
      </div>
  ...
  if (type === 'bars') {
    ...
    return (
      <div className="absolute inset-0 flex items-end justify-center pb-8 gap-[3px]" aria-hidden="true">
        {bars.map((h, i) => (
          <motion.div
            key={i}
    ...
    return (
      <div className="absolute inset-0 flex items-center justify-center" aria-hidden="true">
        <div className="relative w-20 h-20">
          ...
          >
            <div className="w-3 h-3 rounded-full bg-[#B6F34A]/20" />
          </motion.div>
        </div>
      </div>
    )
    ...
    <div className="absolute inset-0 flex items-center justify-center" aria-hidden="true">
      <div className="grid grid-cols-3 gap-1.5">
        {Array.from({ length: 9 }).map((_, i) => (
          <motion.div
            key={i}
            className="w-3 h-3 bg-white/[0.04]"
