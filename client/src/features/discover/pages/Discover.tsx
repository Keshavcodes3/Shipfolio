...
import DiscoverNeeds from '../components/DiscoverNeeds'
...
type DiscoverTab = 'projects' | 'developers' | 'needs'

export default function Discover() {
  const [activeTab, setActiveTab] = useState<DiscoverTab>('projects')
  ...
  const { data: builders } = useFeaturedBuilders()
  ...
  return (
    <SidebarShell>
      <SEO
        ...
        description="Browse projects from builders worldwide. Find inspiration, discover new tools, and see what people are shipping."
        url="/discover"
      />
        ...
        <div className="max-w-[1200px] mx-auto px-5 md:px-8">
          ...
          {featured && (
            ...
            >
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-[10px] uppercase tracking-[0.3em] text-[#B6F34A] font-mono font-medium">PROJECT OF THE WEEK</span>
                <div className="flex-1 h-[1px] bg-white/[0.04]" />
              </div>
              <Link to={`/showcase/${featured.id}`} className="group block border border-white/[0.06] p-8 hover:border-[#B6F34A]/20 transition-colors duration-300">
                <div className="flex flex-col lg:flex-row lg:items-center gap-6">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[15px] text-[#8A8F89] leading-relaxed mb-4 max-w-[480px]">{featured.description}</p>
                    <div className="flex flex-wrap gap-2 mb-4">
                      {featured.technologies.slice(0, 4).map((t) => (
                        <span key={t.technology.id} className="text-[11px] text-[#555B55] border border-white/[0.06] px-2.5 py-1 font-mono">{t.technology.name}</span>
                      ))}
                    </div>
                    <div className="flex items-center gap-3 text-[12px] text-[#555B55]">
                      ...
                      {(featured as any).githubRepo?.stars > 0 && (
                        <>
                          ...
                          <span className="flex items-center gap-1"><Star className="w-3 h-3" />{(featured as any).githubRepo.stars}</span>
                        </>
                      )}
                    </div>
                  </div>
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-6 py-8">
            <div className="flex items-center gap-1 border-b border-white/[0.06]">
              {(['projects', 'developers', 'needs'] as DiscoverTab[]).map((tab) => (
                <button
                  key={tab}
                  onClick={() => { setActiveTab(tab); setSearch('') }}
                  className="relative shrink-0 py-3 px-1 mr-4"
                >
                  <span className={`text-[12px] font-mono uppercase tracking-[0.16em] transition-colors duration-200 ${activeTab === tab ? 'text-[#F5F7F2]' : 'text-[#555B55] hover:text-[#8A8F89]'}`}>
                    {tab}
                  </span>
                  {activeTab === tab && (
                    <motion.div layoutId="discover-tab" className="absolute bottom-0 left-0 right-0 h-[1px] bg-[#B6F34A]" transition={{ duration: 0.25, ease }} />
                  )}
                </button>
              ))}
            </div>
            ...
            <div className="relative flex-1 max-w-[360px]">
              ...
              <input
                ...
                placeholder={activeTab === 'projects' ? 'Search projects, tech...' : activeTab === 'developers' ? 'Search builders...' : 'Search needs, projects...'}
              ...
              <div className="flex items-center gap-2 sm:ml-auto">
                {(['updatedAt', 'createdAt'] as const).map((s) => (
                  <button
                    key={s}
                    onClick={() => setSortBy(s)}
                    className={`text-[10px] uppercase tracking-[0.15em] font-mono px-3 py-1.5 border transition-colors ${sortBy === s ? 'border-[#B6F34A]/30 text-[#B6F34A] bg-[#B6F34A]/[0.05]' : 'border-white/[0.06] text-[#555B55] hover:text-[#8A8F89]'}`}
                  >
          ...
          {activeTab === 'projects' ? (
            <section className="pb-16">
              <div className="flex items-center gap-3 mb-6">
                ...
                <span className="text-[10px] uppercase tracking-[0.3em] text-[#B6F34A] font-mono font-medium">ALL PROJECTS</span>
              ...
              {isLoading ? <Skeleton /> : filtered.length > 0 ? (
                ...
                <div className="py-20 text-center">
                  <p className="text-[14px] text-[#555B55]">{search ? 'No projects match.' : 'No projects yet.'}</p>
                </div>
              )}
            </section>
            ...
            <section className="pb-16">
              <div className="flex items-center gap-3 mb-6">
                <span className="text-[10px] uppercase tracking-[0.3em] text-[#B6F34A] font-mono font-medium">BUILDERS</span>
                <span className="text-[10px] text-[#303530] font-mono">({filteredBuilders.length})</span>
                <div className="flex-1 h-[1px] bg-white/[0.04]" />
              </div>
              {filteredBuilders.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {filteredBuilders.map((builder, i) => (
                    ...
                    >
                      <Link to={`/profile/${builder.username}`} className="block border border-white/[0.06] p-5 hover:border-[#B6F34A]/20 transition-colors group">
                        <div className="flex items-start gap-3 mb-3">
                          <div className="w-10 h-10 rounded-full bg-white/[0.06] border border-white/[0.08] flex items-center justify-center text-[#8A8F89] font-mono font-bold text-[14px] shrink-0">
                            {builder.username[0].toUpperCase()}
                          </div>
                          <div className="min-w-0">
                            <h4 className="text-[14px] font-bold text-[#F5F7F2] tracking-[-0.02em] truncate group-hover:text-[#B6F34A] transition-colors">{builder.displayName ?? builder.username}</h4>
                            <p className="text-[11px] text-[#555B55]">@{builder.username}</p>
                          </div>
                        </div>
                        {builder.bio && <p className="text-[12px] text-[#8A8F89] line-clamp-2 mb-3">{builder.bio}</p>}
                        <div className="flex items-center gap-4 text-[10px] text-[#303530] font-mono">
                ...
                <div className="py-20 text-center">
                  <p className="text-[14px] text-[#555B55]">{search ? 'No builders match.' : 'No builders yet.'}</p>
                </div>
              )}
            </section>
          ) : (
          ...
          {/* CTA */}
          <section className="py-20 md:py-28 border-t border-white/[0.06] mt-12">
            <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, ease }} className="text-center">
              <p className="text-[10px] uppercase tracking-[0.25em] text-[#303530] font-mono mb-4">NOTHING TO DISCOVER?</p>
              <p className="text-[clamp(1.2rem,2.5vw,1.8rem)] font-bold tracking-[-0.03em] text-[#F5F7F2] mb-3">Build something.</p>
              <p className="text-[14px] text-[#555B55] mb-8 max-w-[360px] mx-auto">Your next project could belong in this room.</p>
              <Link to="/projects/new" className="relative group inline-flex items-center gap-2 px-6 py-3 text-[11px] font-medium uppercase tracking-[0.1em] text-[#080A08] overflow-hidden">
